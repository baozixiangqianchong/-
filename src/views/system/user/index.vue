...
function handleQuery() {
  ...
  let queryData = {
    ...
    status:
      queryParams.status == "0" ? 0 : queryParams.status == "1" ? 1 : null,
  ...
  getUserPage(queryData)
    .then(({ data }) => {
      pageData.value = data.Users;
...
async function loadRoleOptions() {
  getRolePage({
    ...
    let res = [];
    for (let item of response.data.all_roles) {
      let obj = {
        label: item.gormadapter_casbin_rule.V1,
        value: item.gormadapter_casbin_rule.V0,
      };
      res.push(obj);
    }
    roleList.value = res;
 ...
 * @param id 用户ID
 */
async function openDialog(type: string, id?: number, row) {
  dialog.visible = true;
  dialog.type = type;
  if (dialog.type === "user-form") {
    ...
    if (id) {
      ...
      // });
      row.gender = row.genderLabel == "男" ? 1 : 2;
      ...
      getRolePage({
        ...
        for (let item of res) {
          if (item.label == row.roleNames.trim()) {
            formData.roleIds?.push(item.value);
          }
        }
...
function handleDelete(id?: number) {
  ...
  ElMessageBox.confirm("确认删除用户?", "警告", {
    ...
    let idsArr = userIds.split(",");
    let newIdsArr = idsArr.map((item) => {
      return item * 1;
    });
    let str = "?";
    newIdsArr.forEach((item, index) => {
      if (index == 0) {
        str = str + `ids=${item}`;
      } else {
        str = str + `&ids=${item}`;
            ...
            <el-table-column label="操作" fixed="right" width="220">
              <template #default="scope">
                ...
                <el-button
                  ...
                  link
                  size="small"
                  @click="openDialog('user-form', scope.row.id, scope.row)"
